...
describe('RP2040', () => {
  it(`should initialize PC and SP according to bootrom's vector table`, () => {
    ...
    expect(rp2040.core.SP).toEqual(0x20041f00);
    expect(rp2040.core.PC).toEqual(0xee);
  });
  ...
  describe('exceptionEntry and exceptionReturn', () => {
    it('should execute an exception handler and return from it correctly', () => {
      ...
      expect(rp2040.core.IPSR).toEqual(EXC_INT1);
      expect(rp2040.core.PC).toEqual(INT1_HANDLER + 2);
      expect(rp2040.core.registers[r0]).toEqual(0x55);
      rp2040.step(); // BX lr
      // Exception handler should return at this point.
      expect(rp2040.core.PC).toEqual(0x10004000);
      expect(rp2040.core.registers[r0]).toEqual(0x44);
      expect(rp2040.core.IPSR).toEqual(0);
    });
    ...
    it('should return correctly from exception with POP {lr}', () => {
      ...
      expect(rp2040.core.registers[r4]).toEqual(42);
      rp2040.step(); // pop {r4, r5, r6, pc}
      // Exception handler should return at this point.
      expect(rp2040.core.PC).toEqual(0x10004000);
      expect(rp2040.core.registers[r4]).toEqual(105);
      expect(rp2040.core.IPSR).toEqual(0);
    });
    ...
    it('should clear the pending interrupt flag in exceptionEntry() for user IRQs (> 25)', () => {
      ...
      expect(rp2040.core.pendingInterrupts).toEqual(INT31);
      // Exception handler should start at this point.
      rp2040.step(); // nop
      expect(rp2040.core.pendingInterrupts).toEqual(0); // interrupt flag has been cleared
  ...
  describe('NVIC registers', () => {
    it('writing to NVIC_ISPR should set the corresponding pending interrupt bits', () => {
      ...
      expect(rp2040.core.pendingInterrupts).toBe(0x11);
    });
    ...
    it('writing to NVIC_ISER should set the corresponding enabled interrupt bits', () => {
      ...
      expect(rp2040.core.enabledInterrupts).toBe(0x11);
    });
    ...
    it('writing to NVIC_ICER should clear corresponding enabled interrupt bits', () => {
      ...
      expect(rp2040.core.enabledInterrupts).toBe(0xef);
    });
    ...
    it('should update the interrupt levels correctly when writing to NVIC_IPR3', () => {
      ...
      const { interruptPriorities } = rp2040.core;
    ...
    it('should return the correct interrupt priorities when reading from NVIC_IPR5', () => {
      ...
      rp2040.core.interruptPriorities[1] = 0x001fffff; // interrupts 0 ... 20
      rp2040.core.interruptPriorities[2] = 0x00200000; // interrupt 21
      rp2040.core.interruptPriorities[3] = 0xffc00000; // interrupt 22 ... 31
      // Set the priority of interrupt number 14 to 2
